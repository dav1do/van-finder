...
pub async fn van_camper(
    ...
    let mut skip = 0;
    let limit = 12;
    let last_id = previous.map(|i| i.id).unwrap_or_default();
    let mut new_hw: Option<HighwaterMark> = None;
    let mut new_van_info: Vec<VanSummary> = Vec::new();
    // current impl is just paginate limit at a time, stop when we get nothing or fewer than our limit
    // could try to figure out how many we need up front, and spawn a task for each request (e.g. using a tokio TaskSet)
    // giving them a channel to send back their results, and then collect them all at the end. Could even pre-allocate _some_ runners (e.g. 4)
    // and each one start at limit * (1..=4) and go up by limit * 4 each time. This would get us done ~4x faster since it's mostly
    // sitting around waiting for the IO. tokio tasks are super light so it wouldn't require more or faster cores to be beneficial
    loop {
        ...
        let mut last_loop = van_data.data.is_empty() || van_data.total < skip;
            ...
            if van.id.to_string() == last_id {
                ...
                new_van_info.push(VanSummary {
                    ...
                    summary: format!(
                        ...
                        van.seats,
                        van.sleeps,
                        van.place.place_name,
                        van.place.postal_code
                    ),
                    price: van.price,
                    miles: van.odometer,
                    status,
